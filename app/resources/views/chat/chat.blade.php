...
<button id="refresh-qr-btn" class="px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-500 transition">
    Reload Data
</button>
    ...
    <div class="p-6 border-2 border-dashed border-gray-300 rounded-lg shadow-sm bg-white">
        <div class="flex flex-col md:flex-row items-center md:items-start md:justify-between">
            ...
            <div class="md:w-2/3 md:pl-6 text-center md:text-left">
                ...
                <!-- tombol diganti -->
                <button id="manual-refresh-btn" class="inline-flex items-center gap-2 px-4 py-2 border border-gray-700 text-gray-700 rounded-lg hover:bg-gray-700 hover:text-white transition">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9M20 20v-5h-.581m-15.357-2a8.003 8.003 0 0115.356-2"/>
                    </svg>
                    Minta QR Baru
                </button>
            </div>
...
async function loadQR() {
    try {
        ...
        // Kalau QR muncul berarti koneksi terputus
        if (data.qr) {
            connected = false;
            connectionStatus.textContent = 'Terputus dari WhatsApp';
...
document.getElementById('refresh-qr-btn').addEventListener('click', loadStatus);
